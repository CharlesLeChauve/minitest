...
void nextToken(void) {
    ...
    if (strcmp(nextInput, "|") == 0) {
        currentToken = PIPE;
    } else if (isCommand(nextInput)) { // Suppose une fonction qui vérifie si l'entrée est une commande.
        currentToken = COMMAND;
    } else if (isArgument(nextInput)) { // Suppose une fonction qui vérifie si l'entrée est un argument.
